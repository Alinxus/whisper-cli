...
export class TerminalUI {
  ...
  // Print a summarized scan report (markdown style)
  printSummaryReport(report) {
    if (this.options.quiet) return;
    // Print only the summary section of the markdown report
    const summaryStart = report.indexOf('┌─────────────────────────────────────────────────────────┐');
    if (summaryStart !== -1) {
      console.log(report.slice(summaryStart));
    } else {
      // Fallback: print the whole report
      console.log(report);
    }
  }
