...
export class WhisperCLI {
  ...
  async scan(path, options) {
    ...
    try {
      ...
      if (aiErrorOccurred) {
        console.log(chalk.yellow('⚠️  AI analysis was unavailable for some or all files. Results may be limited.'));
      }
      ...
      if (scanOptions.output) {
        ...
        console.log(`\n📄 Full report saved to ${scanOptions.output}`);
      } else {
...
`;
  ...
  async doctor() {
    ...
    // const isAuthenticated = await this.auth.check(); // Removed AuthManager
    // checks.push({
    //   name: 'Authentication',
